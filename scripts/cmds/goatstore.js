...
// --- Premium module (inlined — no separate premium.js file needed) --------
const premium = (function () {
  const API_BASE = "https://mirai-store.vercel.app";
  // No admin UID hardcoded here anymore. The backend is the single source
  // of truth for who's admin — every premium call is authorized purely by
  // senderId against the backend's own ADMIN_UIDS list. The client just
  // caches whatever adminUid the backend reports back to us.

  // In-memory cache — kept so every check below (called synchronously, all
  // over goatstore.js) stays a plain sync function. Source of truth is
  // MongoDB on the backend; this is just a warm local mirror of it.
  ...
  function refresh(force = false) {
    ...
    _refreshing = (async () => {
      try {
        // No senderId sent — this is a plain read, the backend decides
        // what's public. Whatever adminUid it reports is cached below.
        ...
        // Network/API hiccup — keep serving the last known-good cache.
      } finally {
        _refreshing = null;
      }
      return state;
  ...
  function isPremiumCmd(cmdName, author) {
    // Explicit cmd-name rule wins first; either match makes it premium —
    // author check still runs even when a name was supplied but didn't match.
    if (cmdName) {
  ...
  async function mutate(action, value, senderId) {
    try {
      // The backend re-checks senderId against its own ADMIN_UIDS list —
      // nothing here can bypass that. senderId must be the real caller's
      // id; there's no local admin fallback to substitute anymore.
  ...
  // Warm the cache immediately on load; goatstore.js's onLoad adds the
  // recurring poll on top of this (same stagger convention as its other
  // background timers).
  refresh(true);
